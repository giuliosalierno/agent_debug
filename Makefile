...
deploy:
	...
	(uv export --no-hashes --no-header --no-dev --no-emit-project --no-annotate > agent/app_utils/.requirements.txt 2>/dev/null || \
	uv export --no-hashes --no-header --no-dev --no-emit-project > agent/app_utils/.requirements.txt) && \
...
register-gemini-enterprise:
	@uvx agent-starter-pack@0.40.0 register-gemini-enterprise