...
os.environ.setdefault("GOOGLE_CLOUD_LOCATION", os.environ.get("GOOGLE_CLOUD_REGION", "us-central1"))
...
BUCKET_NAME = os.environ.get(
    "BUCKET_NAME", f"{project_id}-mova-artifacts"
)
...
try:
    if not bucket.exists():
        print(f"[gcs] Bucket {BUCKET_NAME} does not exist, creating...", flush=True)
        bucket.create(location=os.environ.get("GOOGLE_CLOUD_LOCATION"))
        print(f"[gcs] Bucket {BUCKET_NAME} created successfully.", flush=True)
except Exception as e:
    print(f"[gcs] Error checking/creating bucket: {e}", flush=True)
